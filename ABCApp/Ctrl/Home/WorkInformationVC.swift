...
class WorkInformationVC: BaseVC {
    ...
    override func viewDidLoad() {
        ...
        self.titlelabel.text = "Work Information"
            
        self.getWorklInfoMationAction()
        
        self.workStartStr = DeviceInfoManager.getCurrentMillis()
        // Do any additional setup after loading the view.
    }
    
    
    func getWorklInfoMationAction(){
        
        let params = ["campaign":self.productId]
        
        AppNetHelper.post_inevitableErasure(params: params) { succ in
...
//                if (infoItem?.reelection == "Payday" || infoItem?.reelection == "Company Address"){
                    infoItem?.Selwrinkles = infoItem?.wrinkles
                infoItem?.Selwrinkles = infoItem?.wrinkles
//                }
    ...
    @objc func confirmBtnAction(){
        ...
        var postParams:[String:String] = ["cover":AppClassUtilsHelper.getRandomeStrAction(count: 1), "caressing":AppClassUtilsHelper.getRandomeStrAction(count: 5), "campaign":self.productId]
        
        for conModel:PersonInfoModel? in self.dataArr {
            
            if (conModel?.crept == "1f"){
                postParams[conModel?.marched! ?? "nothing"] = conModel?.Selwrinkles
            }else{
        ...
        AppNetHelper.post_comesHolidayAction(params: postParams) { succ in
            ...
            let postParams:[String:String] = [
                ...
                "triangle": "6",  // 上报场景类型：1、注册 2、认证选择 3、证件信息 4、人脸照片 5、个人信息 6、工作信息 7、紧急联系人 8、银行卡信息9、开始申贷 10、结束申贷
...
extension WorkInformationVC :UITableViewDelegate, UITableViewDataSource{
        ...
        if (conModel.crept == "1f") { //  enum
            ...
            for itemModel:PersonInfoModel? in conModel.juliette {
                let action = UIAlertAction(title: itemModel!.formed, style: .default) { [weak self] action in
                    ...
                    if (itemModel!.juliette.count != 0){
                        
                        let alert = UIAlertController(title: conModel.reelection, message: "", preferredStyle: .actionSheet)

                        for subItemModel:PersonInfoModel in itemModel!.juliette {
                            let action = UIAlertAction(title: subItemModel.formed, style: .default) { [weak self] action in
                                conModel.swelled = (itemModel!.formed ?? "") + "/" + (subItemModel.formed ?? "")
                                conModel.Selwrinkles = (itemModel?.wrinkles ?? "") + "|" + (subItemModel.wrinkles ?? "")
                                self?.conTableView.reloadData()
                        ...
                        conModel.Selwrinkles = itemModel!.wrinkles
                        self?.conTableView.reloadData()
                    }
